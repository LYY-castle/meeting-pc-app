...
SIPml.isScreenShareSupported = function() {
  ...
  return (
    SIPml.getWebRtcType() === 'w4a' ||
    (navigator.userAgent.match('Chrome') && parseInt(navigator.userAgent.match(/Chrome\/(.*) /)[1]) >= 26)
  )
}
...
SIPml.init = function(successCallback, errorCallback) {
  ...
  if (SIPml.b_webrtc_supported && SIPml.b_have_media_stream) {
    ...
    if (errorCallback) {
      var s_description = !SIPml.b_webrtc_supported
        ? 'WebRTC not supported'
        : !SIPml.b_have_media_stream
...
SIPml.Stack = function(o_conf) {
  ...
  if (!SIPml.isWebSocketSupported()) {
    ...
    i_port =
      (o_conf.enable_rtcweb_breaker || (window.location && window.location.protocol == 'https:') ? 10062 : 10060) +
  ...
  this.o_stack.on_event_invite = function(e) {
    ...
    var _setStream = function(o_view, o_stream, b_audio) {
      if (o_view) {
        ...
        return (
          (b_audio && o_stream && o_stream.getAudioTracks().length > 0) ||
          (!b_audio && o_stream && o_stream.getVideoTracks().length > 0)
        )
      }
    }
...
SIPml.Stack.prototype.setConfiguration = function(o_conf) {
  if (o_conf.realm && !tsk_string_is_string(o_conf.realm)) {
    throw new Error(
      "ERR_INVALID_PARAMETER_TYPE: '" + typeof o_conf.realm + "' not a valid type for realm. String is expected"
    )
  }
  if (o_conf.impi && !tsk_string_is_string(o_conf.impi)) {
    throw new Error(
      "ERR_INVALID_PARAMETER_TYPE: '" + typeof o_conf.impi + "' not a valid type for impi. String is expected"
    )
  }
  if (o_conf.impu && !tsk_string_is_string(o_conf.impu)) {
    throw new Error(
      "ERR_INVALID_PARAMETER_TYPE: '" + typeof o_conf.impu + "' not a valid type for impu. String is expected"
    )
  }
  if (o_conf.password && !tsk_string_is_string(o_conf.password)) {
    throw new Error(
      "ERR_INVALID_PARAMETER_TYPE: '" + typeof o_conf.password + "' not a valid type for password. String is expected"
    )
  }
  if (o_conf.display_name && !tsk_string_is_string(o_conf.display_name)) {
    throw new Error(
      "ERR_INVALID_PARAMETER_TYPE: '" +
        typeof o_conf.display_name +
        "' not a valid type for display_name. String is expected"
    )
  }
  if (o_conf.websocket_proxy_url && !tsk_string_is_string(o_conf.websocket_proxy_url)) {
    throw new Error(
      "ERR_INVALID_PARAMETER_TYPE: '" +
        typeof o_conf.websocket_proxy_url +
        "' not a valid type for websocket_proxy_url. String is expected"
    )
  }
  if (o_conf.outbound_proxy_url && !tsk_string_is_string(o_conf.outbound_proxy_url)) {
    throw new Error(
      "ERR_INVALID_PARAMETER_TYPE: '" +
        typeof o_conf.outbound_proxy_url +
        "' not a valid type for outbound_proxy_url. String is expected"
    )
  }
  if (o_conf.sip_headers && !(o_conf.sip_headers instanceof Array)) {
    throw new Error(
      "ERR_INVALID_PARAMETER_TYPE: '" +
        typeof o_conf.sip_headers +
        "' not a valid type for sip_headers. Array is expected"
    )
  }
  ...
  var o_video_size = o_conf.video_size
    ? o_conf.video_size
    : { minWidth: undefined, minHeight: undefined, maxWidth: undefined, maxHeight: undefined }
  ...
  o_stack.set(
    tsip_stack.prototype.SetPassword(o_conf.password),
    ...
    tsip_stack.prototype.SetSecureTransportEnabled(
      b_rtcweb_breaker_enabled || (window.location && window.location.protocol == 'https:')
    ), // always use secure transport when RTCWebBreaker or https://
  ...
  if (o_conf.sip_headers) {
    o_conf.sip_headers.forEach(function(o_header) {
      if (
        o_header &&
        !tsk_string_is_null_or_empty(o_header.name) &&
        (!o_header.value || tsk_string_is_string(o_header.value))
      ) {
        o_stack.set(tsip_stack.prototype.SetHeader(o_header.name, o_header.value))
      }
    })
  }
...
SIPml.Session.prototype.setConfiguration = function(o_conf) {
  ...
  if (this instanceof SIPml.Session.Call) {
    ...
    o_session.set(
      ...
      tsip_session.prototype.SetVideoSize(
        o_conf.video_size
          ? o_conf.video_size
          : { minWidth: undefined, minHeight: undefined, maxWidth: undefined, maxHeight: undefined }
      ),
  ...
  if (o_conf.sip_headers) {
    o_conf.sip_headers.forEach(function(o_header) {
      if (
        ...
        o_session.set(tsip_session.prototype.SetHeader(o_header.name, o_header.value))
      }
    })
  }
  // caps
  if (o_conf.sip_caps) {
    o_conf.sip_caps.forEach(function(o_cap) {
      if (o_cap && !tsk_string_is_null_or_empty(o_cap.name) && (!o_cap.value || tsk_string_is_string(o_cap.value))) {
        o_session.set(tsip_session.prototype.SetCaps(o_cap.name, o_cap.value))
      }
    })
  }
  // expires
...
SIPml.Session.Subscribe.prototype.subscribe = function() {
  return this.o_session.subscribe()
}
/**
Unsubscribe.
