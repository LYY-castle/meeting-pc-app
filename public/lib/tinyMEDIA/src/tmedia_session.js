...
tmedia_session_mgr.prototype.set_stream_local = function(o_stream) {
  ...
  this.callback(
    o_stream ? tmedia_session_events_e.STREAM_LOCAL_ADDED : tmedia_session_events_e.STREAM_LOCAL_REMOVED,
    tmedia_type_e.VIDEO
  )
}
...
tmedia_session_mgr.prototype.set_stream_remote = function(o_stream) {
  ...
  this.callback(
    o_stream ? tmedia_session_events_e.STREAM_REMOTE_ADDED : tmedia_session_events_e.STREAM_REMOTE_REMOVED,
    tmedia_type_e.VIDEO
  )
}
...
tmedia_session_mgr.prototype.load_sessions = function() {
  ...
  if (this.ao_sessions.length == 0 || this.b_media_type_changed) {
    ...
    for (var i = 0; i < tmedia_session_mgr.prototype.__ao_supported_media.length; ++i) {
      ...
      if (e_type.i_id & this.e_type.i_id && !this.has_media(e_type)) {
        // we don't have a session with this media type yet
        if ((o_session = tmedia_session.prototype.Create(e_type, this))) {
          this.ao_sessions.push(o_session)
        }
      } else if (!(e_type.i_id & this.e_type.i_id) && this.has_media(e_type)) {
        // we have media session from previous call (before update)
...
tmedia_session_mgr.prototype.set_ro = function(o_sdp, b_is_offer) {
  ...
  if ((o_hdr_C = o_sdp.get_header(tsdp_header_type_e.C)) && o_hdr_C.s_addr) {
    b_is_loopback_address =
      (tsk_string_iequals('IP4', o_hdr_C.s_addrtype) && tsk_string_iequals('127.0.0.1', o_hdr_C.s_addr)) ||
  ...
  if (this.sdp.o_lo) {
    ...
    if (e_new_mediatype == tmedia_type_e.VIDEO && this.e_type == tmedia_type_e.SCREEN_SHARE) {
      // "SCREEN_SHARE" will be identified in the SDP as Video when using get_media_type()
  ...
  if (
    this.b_started &&
    !(this.is_roap() || this.is_jsep()) &&
    ((!b_is_hold_resume && !b_is_loopback_address) || b_is_mediatype_changed)
  ) {
    if ((i_ret = this.stop())) {
...
if (!window.__b_release_mode) {
  tmedia_api_add_js_scripts(
    'head',
    'lib/tinyMEDIA/src/tmedia_session_jsep.js',
    'lib/tinyMEDIA/src/tmedia_session_roap.js',
    'lib/tinyMEDIA/src/tmedia_session_ghost.js'
  )
}
