                ...
                /*navigator.getUserMedia(
                 ...
                 if(this.e_type == tmedia_type_e.SCREEN_SHARE){
                        navigator.mediaDevices.getDisplayMedia(
                            {
                                audio: false,
                                video: !!(this.e_type.i_id & tmedia_type_e.VIDEO.i_id) ? true : false
                            }).then(stream => {
                                tmedia_session_jsep01.mozThis ? tmedia_session_jsep01.onGetUserMediaSuccess : tmedia_session_jsep01.onGetUserMediaSuccess(stream, This);
								stream.getVideoTracks()[0].onended = function () {sharePhone.sipUnRegister();};
                            }).catch(error => {
                                tmedia_session_jsep01.mozThis ? tmedia_session_jsep01.onGetUserMediaError : tmedia_session_jsep01.onGetUserMediaError(error, This);
                        });
                 }else{
                    navigator.getUserMedia(
                        {
                            audio: true, // IMPORTANT: Chrome '28.0.1500.95 m' doesn't support using audio with screenshare
                            video: !!(this.e_type.i_id & tmedia_type_e.VIDEO.i_id) ? o_video_constraints : false, // "SCREEN_SHARE" contains "VIDEO" flag -> (VIDEO & SCREEN_SHARE) = VIDEO
                            data: false
                        },
                        tmedia_session_jsep01.mozThis ? tmedia_session_jsep01.onGetUserMediaSuccess : function (o_stream) { tmedia_session_jsep01.onGetUserMediaSuccess(o_stream, This); },
