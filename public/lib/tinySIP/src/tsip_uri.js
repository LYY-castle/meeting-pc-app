...
function tsip_uri_make_valid(s_uri, s_domain) {
  ...
  if (
    tsk_string_index_of(s_uri, s_uri.length, 'sip:') != 0 &&
    tsk_string_index_of(s_uri, s_uri.length, 'sips:') != 0 &&
    !b_teluri
  ) {
    s_uri = 'sip:' + s_uri
  }
  if (!b_teluri && tsk_string_index_of(s_uri, s_uri.length, '@') == -1) {
    /* no domain name in tel: uri */
    s_uri += '@' + s_domain
  }
  return tsip_uri.prototype.Parse(s_uri)
}
...
function tsip_uri_compare_parameter(o_uri_1, o_uri_2, s_pname) {
  ...
  if (
    ...
    ((o_param_1 && !o_param_2) ||
      (!o_param_1 && o_param_2) ||
      !tsip_uri_striequals(o_param_1.s_value, o_param_2.s_value))
  ) {
    return -3
  }
  return 0
...
function tsip_uri_compare(o_uri_1, o_uri_2) {
  if (o_uri_1 && o_uri_2) {
    ...
    /* RFC 3261 - 19.1.4 URI Comparison
		...
		*/
    if (
      !tsk_string_equals(o_uri_1.s_scheme, o_uri_2.s_scheme) ||
    ...
    /* Is there parameters */
    if (
      (!o_uri_1.ao_params && !o_uri_2.ao_params) ||
      (o_uri_1.ao_params.length == 0 && o_uri_2.ao_params.length == 0)
    ) {
      return 0
    }
...
if (!window.__b_release_mode) {
  tsip_api_add_js_scripts('head', 'lib/tinySIP/src/parsers/tsip_parser_uri.js')
}
